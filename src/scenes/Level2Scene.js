...
export class Level2Scene extends Phaser.Scene {
    ...
    preload() {
        // Load background music (same as Level 1, use absolute path for Vite)
    ...
    collectPringle(pringle) {
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                ...
                // On mobile: make overlay tapable
                if (this.isMobile) {
                    bgRect.setInteractive({ useHandCursor: false });
                    bgRect.once('pointerdown', () => {
                ...
                const nextLevelText = this.add.text(screenX, screenY + 50, this.isMobile ? 'Tap screen for Level 3' : 'Press N for Level 3', {
                    fontSize: '24px',
                    fill: '#00FF00',
                    fontFamily: 'Arial',
                    fontWeight: 'bold',
                ...
                const restartText = this.add.text(screenX, screenY + 90, this.isMobile ? 'Tap to restart' : 'Press R to restart', {
                    fontSize: '24px',
                    fill: '#00FF00',
                    fontFamily: 'Arial',
                    stroke: '#000000',
    ...
    reachFinish(player, finishZone) {
        ...
        bgRect.setDepth(9999);
        bgRect.setOrigin(0.5, 0.5);
        // On mobile: make overlay tapable so tap-for-next-level works
        if (this.isMobile) {
            bgRect.setInteractive({ useHandCursor: false });
            bgRect.once('pointerdown', () => {
                if (this.victoryKeyPressed) return;
    ...
    update() {
        ...
        if ((this.spaceKey.isDown || this.cursors.up.isDown || this.wasd.W.isDown || this.mobileJump) && isOnGround && !this.isLevitating) {
            this.player.setVelocityY(this.jumpSpeed);
        }
